...
export async function getMergeRequestPosts() {
  if (!MERGE_ID)
    return;
  const merge_api_url = `http://localhost:3000/v1/${BUCKET_SLUG}/merge-requests/${MERGE_ID}/objects?read_key=${READ_KEY}&pretty=true&props=slug,title,content,metadata,created_at`;
